...
WME_Assist.Analyzer = function (wazeapi) {
    ...
    var checkStreet = function (bounds, zoom, streetID, obj, attrName, onProblemDetected) {
        ...
        if (detected) {
            ...
            var objCenter = geometry.getBounds().getCenterLonLat().transform(wazeapi.Config.map.projection.remote, wazeapi.map.getProjectionObject());
            var boundsCenter = bounds.clone().getCenterLonLat().transform(wazeapi.Config.map.projection.remote, wazeapi.map.getProjectionObject());
