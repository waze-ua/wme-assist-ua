...
(function () {
    ...
    function run_wme_assist() {
        ...
        function isWazeApiReady() {
            ...
            if (!wazeapi.model)
                return false;
            if (!wazeapi.model.countries)
                return false;
            if (!wazeapi.model.countries.top)
                return false;
            if (!wazeapi.loginManager)
                return false;
            if (!wazeapi.loginManager.user)
                return false;
                    ...
                    new Rule('Incorrect status position', function (text) {
                        ...
                        text = (' ' + text).replace(/([-\s])([^-\s]+)/g,
                            function (all, space, word) {
                                ...
                                if (/^(летия|лет|года|реч?к[аи]|канала?|острова?|стороны|год|съезда|имени|ручей|канавки|за|из|от|км|километр|де|в|к|о|с|у|на|и)$/i.test(word) ||
                                    ...
                                    return space + word.charAt(0).toUpperCase() + word.substr(1);
                        ...
                        if (new RegExp('(^|\\s)(' + wStatus + ')(\\s|$)').test(text)) {
                            ...
                            text = text.replace(new RegExp('(?:\\s*)(.+?)(?:\\s+)(' + wStatus + ')(?=\\s+|$)'),
                                function (all, adj, s) {
                                    if (new RegExp(exAdjW).test(adj))
                                        return all;
                                    if ((!new RegExp(exW).test(adj)) &&
                                        (/^((\s+[^\s]+?(-я|ая|ья|яя|яся))+)$/.test(' ' + adj)))
                                        return all;
                                    return s + ' ' + adj;
                                });
        ...
        var Ui = function (countryName) {
            ...
            $(section)
                ...
                .append($('<ul>').addClass('result-list').css({
                    "height": "250px",
                    "overflow": "auto"
                }));
            addon.appendChild(section);
            ...
            this.addCustomRule = function (title) {
                var thisrule = $('<li>').addClass('result').click(function () {
                    ...
                    $('#assist_custom_rules li.result').css({
                        'background-color': ''
                    });
            ...
            $('#assist_custom_rule_dialog label').css({
                display: 'block'
            });
            $('#assist_custom_rule_dialog input').css({
                display: 'block',
                width: '100%'
            });
