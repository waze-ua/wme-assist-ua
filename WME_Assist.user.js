...
// @require   https://raw.githubusercontent.com/borman84/wme_assist/develop/scaner.js
// @grant     none
...
function run_wme_assist() {
    ...
    var Rules = function (countryName) {
        ...
        var rules_RU = function () {
            ...
            // Питер: Гусева, Комарова, Панфилова, Тарасова, Старцева, Крюкова, Миронова
            // Великий Новгород: Яковлева, Ильина
            // Краткие притяжательные прилагательные похожие на фамилии
            var exAdjW = 'Репищева|Зеленина|Карташихина|Опочинина|Остоумова|Гаврикова|Прасковьина|Усачёва|Бармалеева|Ильмянинова|Остоумова|Перфильева|Плуталова|Подрезова|Полозова|Рашетова|Сегалева|Шамшева|Шарова|Эсперова|Замшина|Куракина|Ольгина|Опочинина|Осокина|Рюхина|Тосина|Веткина';

            return rules_basicRU().concat([
                new Rule('Incorrect status position', function (text) {
                    // Женские статусы
                    var wStatus = 'улица|набережная|дорога|линия|аллея|площадь|просека|автодорога|эстакада|магистраль|дамба|хорда|коса|деревня|переправа|площадка|дорожка';
                    ...
                    var exStatus = 'closed|^\\d+(:|$)|[Дд]убл[её]р|\\d+[AB]|[МАР]\\d|\\d{2}[АКНР]-\\d|Rail|грунтовка|[Пп]лотина|метро|монорельс|ворота|разворот|шлагбаум|трамвай|пути$|Транссиб|Мост|подход|подъезд|обход|Объездная|въезд|выезд|разворот|шлагбаум|слобода|Грейдер|(?: |^)из |(?: |^)от |(?: |^)на |(?: |^)в |(?: |^)к |^под |^с |^от |ж\\/д|ТТК|КАД|ЗСД|АГ?ЗС|^$| - |\\/|,|плотина|снт|:|промзона|паркинг|парковка|Козлова|Гусева|Комарова|Панфилова|Тарасова|Яковлева|Мишина|Старцева|Крюкова|Ильина|Миронова';

                    // Названия улиц похожие на прилагательные
                    var exW = 'Нехая|Тукая|Мая|Барклая|Батырая|Маклая|Бикбая|Амантая|Нечая|Эшпая|Орая|Прикамья|Алтая|Ухсая|Хузангая|Галлая|Николая|Гая|Эркая|Камая|Пченушая|Здоровья|Палантая';

                    // Названия проспектов похожие на прилагательные
                    var exM = 'Расковой|Дуровой|Космодемьянской|строй|Ковалевской|Борисовой|Давлетшиной|Крупской|Шевцовой|Чайкиной|Богомоловой|Савиной|Попковой|Петровой|Ангелиной|Терешковой|Новоселовой|Красноармейской|Гризодубовой|Красноярский рабочий|Цеткин|Молдагуловой|Чайкиной|Цветаевой|Тимофеевой|Дубровиной';
                    ...
                    text = (' ' + text).replace(/([-\s])([^-\s]+)/g,
                        function(all, space, word) {
                            ...
                            if ( /^(летия|лет|года|реч?к[аи]|канала?|острова?|стороны|год|съезда|имени|ручей|канавки|за|из|от|км|километр|де|ти|го)$/i.test(word) 
                                || word.length == 1
