        ...
        var rules_RU = function () {
            return rules_basicRU().concat([
                ...
                new Rule('Missing street type', function (text) {
                    if (/улица|квартал|площадь|дорога|проезд|магистраль|спуск|путепровод|обвод|переулок|шоссе| тракт$|[А-Я][0-9]|[0-9]+$|тупик|мост|набережная|бульвар|площадь|проспект|аллея|метро|кольцо|тоннель|разворот|эстакада|линия|сад|подход|слобода|^на |^в |^к |^под |^с |^от |ж\/д|МКАД|[0-9][0-9][А-Я]:\/|[0-9][0-9]:|^$|[А-Я]\-[0-9]|[0-9]+$/.test(text)) return text;
                    return 'улица ' + text;
                }),
                new Rule('Incorrect street name', function (text) {
                    var text0 = text;
