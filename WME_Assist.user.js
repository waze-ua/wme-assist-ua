        ...
        var rules_RU = function () {
            return rules_basicRU().concat([
                ...
                new Rule('Incorrect status position', function (text) {
                    ...
                    var exStatus = '[А-Я][0-9]|[0-9]+$|метро|монорельс|ворота|разворот|шлагбаум|трамвай|пути|Мост|подход|подъезд|обход|Объездная|въезд|выезд|слобода|городок|посёлок|поселок|Грейдер|^на |^в |^к |^под |^с |^от |ж\\/д|КАД|ЗСД|АГ?ЗС|[0-9][0-9][А-Я]:\\/|[0-9][0-9]:|^$|[А-Я]\\-[0-9]|[0-9]+$| - |\\/|,|плотина';
                    ...
                    var exAdjM = 'Григоров|Посланников|Мошков|Посланников|Тучков|Замятин|Силин|Горсткин|Алымов|Калинкин|Эсперов|Зеленков|Храмов|Матисов|Лештуков|Кокушкин|Дойников|Щербаков|Апраксин|Климов|Майков|Урхов|Сивков|Урюпин|Воронцов|Яшумов|Захаров|Ласточкин';
                    var brackets = '';
                    // Игнорируем
                    ...
                    if ( new RegExp(wStatus).test(text) ) {
                        ...
                        text = text.replace(new RegExp('(' + wStatus + ')(?:\\s+)(\\d+-я)(?!\\s+[^\\s]+[аяьик][ая]( |$)|\\s+(' + wStatus + '))'), '$2 $1');
                    }
                    ...
                    if ( new RegExp(mStatus).test(text) ) {
                        ...
                        text = text.replace(new RegExp('(' + mStatus + ')(?:\\s+)(\\d+-й)(?!\\s+[^\\s]+[иоык][ий]( |$)|\\s+(' + mStatus + '))'), '$2 $1');
                    }
