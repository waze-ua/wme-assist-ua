        ...
        var rules_RU = function () {
            return rules_basicRU().concat([
                ...
                new Rule('Incorrect status position', function (text) {
                    var wStatus = 'улица|набережная|дорога|линия|аллея|площадь|просека|автодорога|эстакада|магистраль|дамба|хорда';
                    var mStatus = 'проспект|переулок|проезд|тупик|бульвар|тракт|объезд|заезд|съезд|просек|взвоз|спуск|переезд|квартал|путепровод|мост|обвод|разворот|шлагбаум|обход|подъезд|микрорайон| сад|сквер|тоннель|парк';
                    var nStatus = 'шоссе|кольцо';
                    var exStatus = '[А-Я][0-9]|[0-9]+$|метро|монорельс|ворота|разворот|шлагбаум|трамвай|пути|Мост|подход|подъезд|обход|Объездная|въезд|выезд|слобода|городок|посёлок|поселок|Грейдер|^на |^в |^к |^под |^с |^от |ж\\/д|КАД|ЗСД|[0-9][0-9][А-Я]:\\/|[0-9][0-9]:|^$|[А-Я]\\-[0-9]|[0-9]+$| - ';
                    ...
                    var exAdjW = 'Репищева|Зеленина';
                    var exAdjM = 'Григоров|Посланников|Мошков|Посланников|Тучков|Замятин|Силин';
                    var brackets = '';
                    ...
                    text = text.replace(new RegExp('(\\d)\\s+(?='+ wStatus + ')'), '$1-я ');
                        ...
                        text = text.replace(new RegExp('(' + wStatus+ '|' + mStatus + ')(.*)'),
                            function(all, status, name){
                                name = name.replace(/ ([^\s]+)/g,
                                    function(all, word){
                                        if (/лет|рек/.test(word)) return ' ' + word;
