    ...
    var Rules = function (countryName) {
        var rules_basicRU = function () {
            return [
                ...
                new Rule('Incorrect street name', function (text) {
                    return text.replace(/(^| )(мкр[.н]?|мк?р?-н)( |$)/, '$1микрорайон$3');
                }),
                new Rule('Incorrect street name', function (text) {
        ...
        var rules_RU = function () {
            return rules_basicRU().concat([
                new Rule('Incorrect status position', function (text) {
                    ...
                    var exStatus = 'closed|^\\d+(:|$)|[Дд]убл[её]р|\\d+[AB]|[МАР]\\d|\\d{2}[АКНР]-\\d|Rail|грунтовка|[Пп]лотина|метро|монорельс|ворота|шлагбаум|трамвай|пути$|Транссиб|Мост|подход|подъезд|обход|въезд|выезд|разворот|шлагбаум|слобода|Грейдер|(?: |^)из |(?: |^)от |(?: |^)на |(?: |^)в |(?: |^)к |^под |^с |^от |ж\\/д|ТТК|КАД|ЗСД|АГ?ЗС|^$| - |\\/|,|плотина|снт|:|[Пп]ромзона|паркинг|парковка|Козлова|Гусева|Комарова|Панфилова|Тарасова|Яковлева|Мишина|Старцева|Крюкова|Ильина|Миронова|Перфильева';
                    ...
                    if ( ! new RegExp('(^|\\s+)(' + wStatus + '|' + mStatus + '|' + nStatus + ')(\\s+|$)').test(text) ) {
                        ...
                        if ( /^Об[ъь]ездная$/.test(text)) {
                            text = 'Объездная дорога';
                        } else
                    ...
                    text = (' ' + text).replace(/([-\s])([^-\s]+)/g,
                        function(all, space, word) {
                            ...
                            if ( /^(летия|лет|года|реч?к[аи]|канала?|острова?|стороны|год|съезда|имени|ручей|канавки|за|из|от|км|километр|де|в|к|о|с|у|на|и)$/i.test(word)
                                || ( space == '-' && /^(лейтенант|майор|полковник|й|я|ти|го|е)$/.test(word) ) )
