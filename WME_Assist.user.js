        ...
        var rules_RU = function () {
            return rules_basicRU().concat([
                ...
                new Rule('Incorrect status position', function (text) {
                    ...
                    var mStatus = 'проспект|переулок|проезд|тупик|бульвар|тракт|объезд|заезд|съезд|просек|взвоз|спуск|переезд|квартал|путепровод|мост|обвод|разворот|шлагбаум|обход|подъезд|микрорайон| сад|сквер|тоннель|парк|проток|канал';
                    var nStatus = 'шоссе|кольцо';
                    var exStatus = '[А-Я][0-9]|[0-9]+$|метро|монорельс|ворота|разворот|шлагбаум|трамвай|пути|Мост|подход|подъезд|обход|Объездная|въезд|выезд|слобода|городок|посёлок|поселок|Грейдер|^на |^в |^к |^под |^с |^от |ж\\/д|КАД|ЗСД|АГ?ЗС|[0-9][0-9][А-Я]:\\/|[0-9][0-9]:|^$|[А-Я]\\-[0-9]|[0-9]+$| - |\\/|,| и ';
                    ...
                    var exAdjM = 'Григоров|Посланников|Мошков|Посланников|Тучков|Замятин|Силин|Горсткин|Алымов|Калинкин|Эсперов|Зеленков|Храмов|Матисов|Лештуков|Кокушкин|Дойников|Щербаков|Апраксин|Климов|Майков|Урхов|Сивков|Урюпин';
                    var brackets = '';
                    // Игнорируем
                    ...
                    text = text.replace(new RegExp('(' + wStatus+ '|' + mStatus + ')([^,]+)'), function(all, status, name){
                        name = name.replace(/[\s\-]([^\s]+)/g, function(all, word){
                            if (/^(летия|лет|года|реки|канала|острова|стороны|год|съезда|имени|ручей|канавки)$/i.test(word))
