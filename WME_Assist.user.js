        ...
        var rules_RU = function () {
            ...
            return rules_basicRU().concat([
                ...
                new Rule('Incorrect status position', function (text) {
                    ...
                    // Мужские статусы
                    var mStatus = 'проспект|переулок|проезд|тупик|бульвар|тракт|объезд|заезд|съезд|просек|взвоз|спуск|разъезд|переезд|квартал|путепровод|мост|обвод|обход|подъезд|сад|сквер|тоннель|туннель|парк|проток|канал|остров|микрорайон|район|городок|посёлок|поселок|км';

                    // Средние статусы
                    var nStatus = 'шоссе|кольцо';

                    // Не нужно добавлять статус или изменять порядок слов
                    var exStatus = '\\d+[AB]|[МАР]\\d|\\d{2}[АКН]-\\d|Rail|грунтовка|[Пп]лотина|метро|монорельс|ворота|разворот|шлагбаум|трамвай|пути|Транссиб|Мост|подход|подъезд|обход|Объездная|въезд|выезд|разворот|шлагбаум|слобода|Грейдер|(?: |^)из |(?: |^)от |(?: |^)на |(?: |^)в |(?: |^)к |^под |^с |^от |ж\\/д|ТТК|КАД|ЗСД|АГ?ЗС|^$| - |\\/|,|плотина|снт|:|промзона|паркинг|парковка|Козлова|Гусева|Комарова|Панфилова|Тарасова|Яковлева';
