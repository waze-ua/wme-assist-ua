        ...
        var rules_RU = function () {
            return rules_basicRU().concat([
                ...
                new Rule('Missing street type', function (text) {
                    if (/улица|квартал|площадь|дорога|проезд|магистраль|спуск|путепровод|обвод|переулок|шоссе| тракт$|[А-Я][0-9]|[0-9]+$|тупик|мост|набережная|бульвар|площадь|проспект|аллея|метро|кольцо|тоннель|разворот|эстакада|линия|сад|подход|подъезд|обход|Объездная|въезд|слобода|городок|посёлок|^на |^в |^к |^под |^с |^от |ж\/д|МКАД|[0-9][0-9][А-Я]:\/|[0-9][0-9]:|^$|[А-Я]\-[0-9]|[0-9]+$| - |микрорайон/.test(text)) return text;
                    return 'улица ' + text;
                }),
                new Rule('Incorrect street name', function (text) {
                    var text0 = text;
                ...
                new Rule('Incorrect street name', function (text) {
                    ...
                    text = text.replace(/(проспект|переулок|проезд|тупик|бульвар|тракт|объезд|заезд|съезд|просек|взвоз|спуск|переезд|квартал|путепровод|мост|обвод|разворот|шлагбаум|обход|подъезд|микрорайон)(\s)(.+[-|и|о|ы]й$)/, '$3 $1');
