...
function run_wme_assist() {
    ...
    var Rules = function (countryName) {
        ...
        var rules_RU = function () {
            ...
            // Питер: Гусева, Комарова, Панфилова, Тарасова
            // Великий Новгород: Яковлева
            // Краткие притяжательные прилагательные похожие на фамилии
            var exAdjW = 'Репищева|Зеленина|Карташихина|Опочинина|Остоумова|Гаврикова|Прасковьина|Усачёва|Бармалеева|Ильмянинова|Крюкова|Миронова|Остоумова|Перфильева|Плуталова|Подрезова|Полозова|Рашетова|Сегалева|Старцева|Шамшева|Шарова|Эсперова|Замшина|Куракина|Ольгина|Опочинина|Осокина|Рюхина|Тосина|Веткина|Ильина';
            ...
            return rules_basicRU().concat([
                ...
                new Rule('Incorrect status position', function (text) {
                    // Женские статусы
                    var wStatus = 'улица|набережная|дорога|линия|аллея|площадь|просека|автодорога|эстакада|магистраль|дамба|хорда|коса|деревня|переправа';

                    // Мужские статусы
                    var mStatus = 'проспект|переулок|проезд|тупик|бульвар|тракт|объезд|заезд|съезд|просек|взвоз|спуск|разъезд|переезд|квартал|путепровод|мост|обвод|обход|подъезд|сад|сквер|тоннель|туннель|парк|проток|канал|остров|микрорайон|район|городок|посёлок|поселок';

                    // Средние статусы
                    var nStatus = 'шоссе|кольцо';

                    // Не нужно добавлять статус или изменять порядок слов
                    var exStatus = '\\d+[AB]|[МАР]\\d|\\d{2}[АКН]-\\d|Rail|грунтовка|[Пп]лотина|метро|монорельс|ворота|разворот|шлагбаум|трамвай|пути|Транссиб|Мост|подход|подъезд|обход|Объездная|въезд|выезд|разворот|шлагбаум|слобода|Грейдер|(?: |^)из |(?: |^)от |(?: |^)на |(?: |^)в |(?: |^)к |^под |^с |^от |ж\\/д|ТТК|КАД|ЗСД|АГ?ЗС|^$| - |\\/|,|плотина|снт|:|промзона|паркинг|парковка|Козлова|Гусева|Комарова|Панфилова|Тарасова|Яковлева|Мишина';

                    // Названия улиц похожие на прилагательные
                    var exW = 'Нехая|Тукая|Мая|Барклая|Батырая|Маклая|Бикбая|Амантая|Нечая|Эшпая|Орая|Прикамья|Алтая|Ухсая|Хузангая|Галлая|Николая|Гая|Эркая';

                    // Названия проспектов похожие на прилагательные
                    var exM = 'Расковой|Дуровой|Космодемьянской|строй|Ковалевской|Борисовой|Давлетшиной|Крупской|Шевцовой|Чайкиной|Богомоловой|Савиной|Попковой|Петровой|Ангелиной|Терешковой|Новоселовой|Тружеников|Артиллеристов|Красноармейской|Энергетиков|Металлургов|конструкторов|авиаторов|Артиллеристов|Большевиков|Ветеранов|Героев|североморцев|Гризодубовой|Декабристов|Космонавтов|Красноярский рабочий|Медиков|Металлистов|Наставников|Нефтянников|Новаторов|Патриотов|Текстильщиков|Стратонавтов|Ударников|Химиков|Художников|Цветоводов|Шахтёров|Энтузиастов|Архитекторов|Сталеваров|Сибиряковцев|Труженников|Дорожников|Профсоюзов|Геофизиков';
                    ...
                    text = text.replace(new RegExp('(\\d)\\s+(?=' + wStatus + ')', 'g'), '$1-я '); // 1 линия
                    text = text.replace(new RegExp('(\\d)\\s+(?=' + mStatus + ')', 'g'), '$1-й '); // 2 проезд
                    ...
                    if ( new RegExp('(^|\\s)(' + wStatus + ')(\\s|$)').test(text) ) {
                        ...
                        text = text.replace(new RegExp('(' + wStatus + ')(?:\\s+)(\\d+-я)(?!\\s+[^\\s]+(?:ая|ья|яя|яся|ка)( |$)|\\s+(' + wStatus + '|Ферма|Авеню|Пристань|Рота))', 'i'), '$2 $1');
                    ...
                    if ( new RegExp('(^|\\s)(' + mStatus + ')(\\s|$)').test(text) ) {
                        ...
                        if ( ! new RegExp(exM, 'i').test(text) ) {
                            ...
                            text = text.replace(
                                new RegExp('(' + mStatus + ')(?:\\s+)([^\\s]+(?:[-иоы]й|ин|[оеё]в))(?=\\s+\\d+-й\\s+Проезд)'), '$2 $1');
                        }
